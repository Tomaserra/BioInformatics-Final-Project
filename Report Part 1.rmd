...
## **Clinical Implementation via REST API**

To make our predictive model accessible to clinicians without requiring R expertise, we developed a RESTful API using the `plumber` package. This allows external systems (Electronic Health Records, web dashboards, research platforms) to send patient methylation data and receive real-time Alzheimer's risk predictions via standard HTTP requests.

### **API Source Code**

The complete API implementation is stored in `API Part 1.r` (see first 25 lines):

```{r api-code-display, echo=FALSE, comment=''}
# Display the first 25 lines of the API source code in the report
if (file.exists("API Part 1.r")) {
  api_code <- readLines("API Part 1.r")
  cat(paste(api_code[1:min(25, length(api_code))], collapse = "\n"))
} else {
  cat("Error: API Part 1.r not found in the working directory.")
}
```
...
The API provides seven endpoints for different functionalities:

### **1. Health Check (`GET /health`)**

Verifies the API is running and the model loaded correctly.

```bash
curl http://localhost:8000/health
```

Returns server status, model availability, and number of required CpG sites.

### **2. Model Information (`GET /model-info`)**

Provides comprehensive model details including:

- Model type and predictive performance (accuracy, sensitivity, specificity)
- Required input format and variables
- List of selected CpG sites
- Human-readable interpretation of performance metrics

### **3. CpG Site Importance (`GET /cpg-importance`)**

Ranks methylation markers by their contribution to predictions:

- Odds ratios for each CpG site
- Effect direction (hypermethylation vs hypomethylation)
- Statistical significance levels
- Top 5 most important markers

### **4. Patient Prediction (`POST /predict`)** **- Core Endpoint**

Generates Alzheimer's risk prediction for individual patients.

**Input format (JSON):**
```json
{
  "M_values": [0.5, -1.2, 2.3, 0.8, -0.5, 1.1, 0.3, -0.9, 1.5, 0.2],
  "age": 75,
  "sex": "M",
  "cell_type": "neuron",
...
- **LOW** (< 30%): The DNA methylation profile matches healthy control tissue. Suggests absence of AD-specific epigenetic alterations.
- **MODERATE** (30-70%): Methylation patterns are intermediate. Correlate with Braak staging and histopathology for confirmation.
- **HIGH** (≥ 70%): The molecular profile is highly consistent with Alzheimer's Disease pathology. Supports a retrospective molecular diagnosis of AD.


### **5. Dataset Statistics (`GET /dataset-stats`)**

Documents training data provenance:

- Dataset source (GSE66351)
- Feature selection methodology (covariate-controlled regression, Bonferroni correction)
- Model validation approach (70/30 train-test split)
- Selected CpG sites list

### **6. Beta to M-value Conversion (`POST /convert-beta-to-m`)**

Helper endpoint for preprocessing methylation data:
...
Applies transformation: $M = \log_2\left(\frac{\beta}{1-\beta}\right)$ with edge case handling.

### **7. API Documentation (`GET /`)**

Landing page with endpoint overview and example curl commands.
...
pr <- plumber::pr("API Part 1.r")
plumber::pr_run(pr, port = 8000)
```

The server starts at `http://localhost:8000` with interactive documentation at `http://localhost:8000/__docs__/`
...
# Step 1: Convert Beta values to M-values
beta_response = requests.post(
    "http://localhost:8000/convert-beta-to-m",
    json={"beta_values": [0.52, 0.68, 0.45, 0.71, 0.38, 
                          0.64, 0.49, 0.58, 0.67, 0.42]}
)
...
### **Required Files:**
1. `alzheimers_model.rds` - Trained logistic regression model
2. `selected_cpgs.rds` - List of `r length(top_cpgs)` selected CpG sites
3. `alzheimers_model_api.rds` - Complete API data package including:
   - Model object
   - CpG site list
   - Coefficient interpretation (β, OR, CI, p-values)
   - Performance metrics (train and test)
   - Beta-to-M conversion function
   - Confusion matrices
...
- **M-values**: Ensures exact number of CpG sites required by model
- **Covariates**: Checks all clinical variables (age, sex, cell_type, region) are present
- **Factor levels**: Validates categorical variables match training data levels
- **Type conversion**: Automatically converts inputs to appropriate types (numeric/factor)

If validation fails, the API returns detailed error messages indicating which fields are missing or incorrect.

### **Preprocessing Pipeline**

For each prediction request:

1. **Parse JSON** input from HTTP body
2. **Validate inputs** (type, range, completeness)
3. **Create data frame** matching model's expected structure
4. **Name M-values** as M_1, M_2, ..., M_n to align with model coefficients
5. **Convert factors** with correct levels
6. **Pass to model** for prediction
7. **Post-process** probability into risk categories
8. **Format response** with clinical interpretation
...
|------------|-------------|----------|
| Invalid JSON | 400 | "Invalid JSON format" + details |
| Missing fields | 400 | List of required fields |
| Wrong data type | 400 | Expected vs. actual type |
| Invalid factor level | 400 | Valid options for categorical variables |
| Wrong array length | 400 | Expected number of M-values |
| Server error | 500 | Generic error message (no internal details exposed) |
...
- **1. Post-mortem training data**: Model trained on brain tissue from deceased patients; generalization to living patients unvalidated
- **2. Single cohort**: Requires validation on independent datasets (GSE59685, GSE80970)
- **3. Brain tissue only**: Clinical utility depends on validating in accessible biofluids (blood, CSF)
...
The API bridges the gap between statistical modeling and clinical application by:

**1. Abstracting complexity**: Clinicians send simple JSON requests, receive interpretable risk assessments  

**2. Automating preprocessing**: Beta-to-M conversion and data validation handled automatically  

**3. Providing clinical context**: Risk stratification and evidence-based recommendations included  

**4. Enabling integration**: Standard HTTP interface compatible with any programming language or system  

**6. Ensuring reliability**: Comprehensive input validation prevents errors  

**Deployment pathway:**

1. Validate on independent post-mortem datasets
2. Test generalization to blood/CSF methylation data  
3. Conduct prospective clinical validation study
4. Seek regulatory approval (FDA clearance)
5. Deploy on secure, compliant infrastructure

This API represents the critical translation step from research finding to clinical decision support tool, making advanced epigenetic analysis accessible at the point of care
