...
The following is a preview of the API implementation stored in `APIFinalProject.r`:
...
if(file.exists("APIFinalProject.r")) {
  ...
  preview_lines <- head(api_code, 25)
  
  # Print the preview
  cat(paste(preview_lines, collapse = "\n"))
  
  # Add a visual indicator that the code is truncated
  cat("\n\n# ... [Rest of code truncated for brevity. See APIFinalProject.r for full implementation] ...")
} else {
  cat("Error: APIFinalProject.r not found.")
}
```
...
All endpoints use try-catch blocks to gracefully handle errors:

- Invalid JSON → 400 Bad Request with explanation
- Missing required fields → 400 with list of required fields
- Mismatched factor levels → 400 with valid options
- Server errors → 500 with error details (without exposing internal code)

---

## **Model Limitations**

- **Post-mortem training data**: Model trained on brain tissue from deceased patients; generalization to living patients unvalidated
- **Single cohort**: Requires validation on independent datasets (GSE59685, GSE80970)
- **Brain tissue only**: Clinical utility depends on validating in accessible biofluids (blood, CSF)

This said, this API represents the critical translation step from research finding to clinical decision support tool, making advanced epigenetic analysis accessible at the point of care.
