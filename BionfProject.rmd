...
The complete API implementation is stored in `APIFinalProject.r`:

```{r api-code-display, echo=FALSE, comment=''}
# Display the first 25 lines of the API source code in the report
if (file.exists("APIFinalProject.r")) {
  api_code <- readLines("APIFinalProject.r")
  cat(paste(api_code[1:min(25, length(api_code))], collapse = "\n"))
} else {
  cat("Error: APIFinalProject.r not found in the working directory.")
}
```
...
- **LOW** (< 30%): Standard follow-up recommended
- **MODERATE** (30-70%): Additional testing suggested (MRI, CSF biomarkers, PET scan)
- **HIGH** (≥ 70%): Comprehensive neurological evaluation strongly recommended
