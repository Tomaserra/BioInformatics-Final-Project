...
This project develops a predictive model for Alzheimer's Disease (AD) using DNA methylation data from post-mortem brain tissue. We use logistic regression to identify which methylation markers, after controlling for demographic and technical confounders, can distinguish AD patients from healthy controls.
...
DNA methylation is an epigenetic modification where methyl groups (CH₃) are added to cytosine bases, typically at CpG dinucleotides. This modification regulates gene expression without changing the DNA sequence.
...
Recent studies have identified differential methylation in AD-related genes including:
...
**CpG sites**: 485,577 sites across the genome
...
## Important Note: Computational Efficiency

This analysis involves fitting **485,577 linear models** (one per CpG site), which is computationally intensive.

**Caching system implemented:**

- **First run**: Takes ~15-20 minutes total
  - Download dataset: ~5-10 min (500MB file)
  - Fit linear models: ~10-15 min
- **Subsequent runs**: Takes ~2 minutes (loads cached results)
...
**For reviewers:** This document can be knitted from scratch without any manual intervention. Just click "Knit" and wait ~15-20 minutes on first run.
...
**Reference**: Du et al. (2010) "Comparison of Beta-value and M-value methods for quantifying methylation levels by microarray analysis"
...
hist(sample_m, breaks=50, main="M-Values", xlab="M-value", col="lightcoral")
par(mfrow=c(1,1))
```
...
We have **485,577 CpG sites** but only **~190 samples**. This creates several problems:
...
**2. Established in epigenome-wide association studies (EWAS)**
...
Without covariates, we might identify CpG sites that differ between AD and controls, but these differences could be due to:
...
By including covariates, we ask: **"Do methylation levels change with AD progression INDEPENDENT of age, sex, cell type, and region?"**
...
**Without covariates**: "CpG site X is different in AD patients"

- Maybe it's just because AD patients are older?

**With covariates**: "CpG site X changes with AD progression, even after accounting for age differences"

- Now we know it's AD-specific!
...
We're testing **485,577 CpG sites**, each at α = 0.05. 

**Expected false positives** under null hypothesis:

$$485,577 \times 0.05 = 24,279 \text{ false positives!}$$

This is unacceptable - we need correction.
...
### Choosing Bonferroni for This Project

**Reasons for choosing Bonferroni:**
...
For exploratory research, FDR would be appropriate. For a clinical prediction tool, Bonferroni is safer.
...
```{r linear-models-grid, fig.height=8, fig.width=12, fig.cap="Linear regression fits for top 6 CpG sites showing relationship with Braak stage"}
# Plot linear models for top 6 CpGs
top_6_cpgs <- top_cpgs[1:min(6, length(top_cpgs))]

# Prepare plotting data
braak_for_plot <- as.numeric(clinical_data_valid$braak_stage)

plot_data_list <- list()
for (cpg in top_6_cpgs) {
...
ggplot(plot_data, aes(x = Braak_stage, y = M_value)) +
  ...
  geom_smooth(method = "lm", se = TRUE, color = "#d62728", fill = "#ff7f0e", 
              alpha = 0.2, linewidth = 1.2) +
  facet_wrap(~CpG, scales = "free_y", ncol = 3) +
  scale_x_continuous(breaks = 0:6) +
  labs(title = "Linear Models: Top 6 CpG Sites vs AD Progression",
...
**Goal**: Build a model that works on **new, unseen data**

**Problem**: Without proper validation, we don't know if our model:

- **Learned patterns** (good!)
- **Memorized training data** (overfitting - bad!)
...
- **Alternative approaches**: Cross-validation (not used here due to small sample size)

### Key Principle

**Never use test data during model development!**

- Don't select features using test data
- Don't tune parameters using test data
- Only evaluate final model on test data
...
train_idx <- createDataPartition(analysis_data$AD_status, p = 0.8, list = FALSE)
...
1. **Rigorous feature selection**: Covariate-controlled linear modeling
2. **Conservative correction**: Bonferroni ensures high confidence
3. **Proper validation**: Train/test split for unbiased evaluation
4. **Comprehensive covariates**: Controls for confounding factors
5. **Statistical best practices**: M-values, sample size considerations
...
- Limited to ~190 samples
- Restricts number of predictors we can include
- May miss weaker but true associations

### 2. Post-Mortem Tissue
...
- Requires brain tissue (not practical for screening)
- Could inform biomarker discovery in blood/CSF
- Provides mechanistic insights for drug targets
...
This project successfully developed a logistic regression model for Alzheimer's Disease prediction using DNA methylation markers from brain tissue.
...
- Model is ready for deployment via API for further testing

**Clinical relevance:**

While this model uses post-mortem brain tissue (limiting immediate clinical use), it:

- Identifies potential therapeutic targets
- Suggests biomarkers for blood-based tests
- Improves our understanding of AD epigenetics
- Provides foundation for future diagnostic tools

---

# References

1. Lunnon K, et al. (2014). Methylomic profiling implicates cortical deregulation of ANK1 in Alzheimer's disease. *Nature Neuroscience*, 17(5), 669-677.

2. Du P, et al. (2010). Comparison of Beta-value and M-value methods for quantifying methylation levels by microarray analysis. *BMC Bioinformatics*, 11, 587.
...
4. Braak H & Braak E (1991). Neuropathological stageing of Alzheimer-related changes. *Acta Neuropathologica*, 82(4), 239-259.

5. Pidsley R, et al. (2013). A data-driven approach to preprocessing Illumina 450K methylation array data. *BMC Genomics*, 14, 293.